...
class FlaggingPlugin extends Gdn_Plugin {
   ...
   protected function AddFlagButton($Sender, $Args, $Context = 'comment') {
      ...
      $FlagName = '<i class="icon-flag"></i> '.T('Flag');
      $FlagLink = Anchor($FlagName, "discussion/flag/{$Context}/{$ElementID}/{$ElementAuthorID}/".Gdn_Format::Url($ElementAuthor)."/{$EncodedURL}", 'FlagContent Popup');
      echo Wrap($FlagLink, 'span', array('class' => 'MItem Blink CommentFlag'));
   }
